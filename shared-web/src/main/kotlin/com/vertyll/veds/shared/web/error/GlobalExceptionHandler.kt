...
/**
 * Turns every failure into the same envelope, whichever service threw it.
 *
 * A caller sees one shape and one set of common keys, so the front end handles a
 * refusal from any service the same way. Only the key changes, and that comes from
 * the service's own catalogue.
 *
 * Registered by [com.vertyll.veds.shared.web.error.ErrorHandlingAutoConfiguration];
 * a service that needs to differ declares its own `@RestControllerAdvice`, which
 * takes precedence for the exceptions it names.
 */
