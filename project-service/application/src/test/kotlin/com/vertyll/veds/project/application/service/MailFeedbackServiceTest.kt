...
/**
 * Mail is sent by another service, and its answer is what closes the invitation saga. The topic
 * carries every service's mail traffic, so this handler sees sagas that are not its own — and
 * acting on one of those would settle a workflow belonging to somebody else.
 */
internal class MailFeedbackServiceTest {
    ...
    /**
     * Every service consumes the same mail topic. A saga id this service does not know belongs to
     * another one, and settling it here would close a workflow this service has no part in.
     */
    @Test
